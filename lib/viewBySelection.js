...
async function viewBySelection(connection) {
    ...
    switch (answer.target) {
        ...
        case "View roles":
            [response] = await connection.query("SELECT * FROM roles ORDER BY title");
            break;
        case "Back":
            return
        default:
...
const select = `SELECT e.id, e.first_name AS "first name",e.last_name AS "last name",r.title,d.name,r.salary
FROM employees AS e
